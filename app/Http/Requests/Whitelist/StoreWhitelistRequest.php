...
class StoreWhitelistRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'server_ids.*' => 'required|integer|distinct|exists:game_servers,id',
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'server_ids.required' => 'At least one server must be selected',
            'server_ids.*.exists' => 'One or more selected servers do not exist',
            'server_ids.*.integer' => 'Invalid server ID format',
            'server_ids.*.distinct' => 'Duplicate servers were selected',
        ];
    }
}
